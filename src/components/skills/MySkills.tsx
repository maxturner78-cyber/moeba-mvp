...
const MySkills: React.FC = () => {
  ...
  // Build nodes from proficiency data, filtered by month slider
  const allNodes: SkillNode[] = useMemo(() => {
    if (!profRows) return [];
    return profRows
      .filter((r) => {
        if (month === 1) return r.last_assessed_week <= 4;
        if (month === 2) return r.last_assessed_week <= 8;
        return true; // month 3 = all
      })
      .map((r) => ({
        id: r.skill_node.slug,
        label: r.skill_node.name,
        cluster: r.skill_node.cluster as SkillNode["cluster"],
        proficiency: r.current_level,
        promotionRequired: r.skill_node.promotion_required,
      }));
  }, [profRows, month]);

  // Build edges, translating node IDs to slugs
  const allEdges: SkillEdge[] = useMemo(() => {
    if (!edgeRows || !profRows) return [];
    // Build id → slug map from proficiency rows
    const idToSlug = new Map<string, string>();
    for (const r of profRows) {
      idToSlug.set(r.skill_node.id, r.skill_node.slug);
    }
    const nodeSlugSet = new Set(allNodes.map((n) => n.id));
    return edgeRows
      .map((e: any) => ({
        source: idToSlug.get(e.source_node_id) ?? "",
        target: idToSlug.get(e.target_node_id) ?? "",
      }))
      .filter((e) => e.source && e.target && nodeSlugSet.has(e.source) && nodeSlugSet.has(e.target));
  }, [edgeRows, profRows, allNodes]);

  const developed = allNodes.filter((n) => n.proficiency > 6).length;
  const developing = allNodes.filter((n) => n.proficiency > 0 && n.proficiency <= 6).length;
  ...
  return (
    <div className="flex flex-col" style={{ height: "calc(100vh - 112px)" }}>
      ...
      <div className="flex items-start justify-between" style={{ marginBottom: 20, flexShrink: 0 }}>
        <div>
          <h1
            ...
            style={{ fontSize: 26, fontWeight: 700, color: "#0F0F0F", letterSpacing: "-0.02em", marginBottom: 4 }}
          >
            My Skills
          </h1>
        ...
        <div className="flex items-center gap-3" style={{ flexShrink: 0 }}>
          <span className="font-mono-data" style={{ fontSize: 12, fontWeight: 500, color: month === 1 ? "#0F0F0F" : "#9CA3AF" }}>
            Month 1
          </span>
          <input
            type="range"
          ...
          />
          <span className="font-mono-data" style={{ fontSize: 12, fontWeight: 500, color: month === 3 ? "#0F0F0F" : "#9CA3AF" }}>
            Month 3
          </span>
        </div>
      </div>
      ...
      <div
        style={{
          flex: 1, minHeight: 600, background: "#FFFFFF", borderRadius: 10,
          border: "1px solid #E8E8E8", boxShadow: "0 1px 3px rgba(0,0,0,0.04), 0 1px 2px rgba(0,0,0,0.02)",
      ...
      >
        {isLoading ? (
          <div className="flex flex-col items-center gap-3">
            <div
              style={{
                width: 28, height: 28, border: "3px solid #E8E8E8",
          ...
          <span style={{ fontSize: 13, color: "#9CA3AF" }}>No skills data for this period yet.</span>
        ) : (
          <SkillsGraph nodes={allNodes} edges={allEdges} />
        )}
      ...
      {/* Summary stats */}
      <div className="flex items-center gap-1 flex-wrap" style={{ marginTop: 16, flexShrink: 0 }}>
        <span className="font-mono-data" style={{ fontSize: 13, fontWeight: 600, color: "#0F0F0F" }}>{allNodes.length}</span>
